...
def kontakt(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Wenn gew��nscht, Kopie an den Absender...
...
def graduierungen(request):
    ...
    mitglieder = Mitglied.public_objects.get_mitglieder().order_by('graduierung', 'graduierung__datum', 'vorname')
#    mitglieder.sort(lambda x, y: cmp(x.aktuelle_graduierung(), y.aktuelle_graduierung()))
    ctx['mitglieder'] = mitglieder
...
def picasa_albums(request):
    ...
    try:
        ...
        albums = service.GetUserFeed(user=ctx['username'])
        ctx['albums'] = albums
        return __create_response(request, ctx, 'bilder.html')
    except:
        raise Http404
...
def video(request, vid=None):
    ...
    return __create_response(request, ctx, 'videos-simple.html')
    ...
    feed = ys.GetYouTubeVideoFeed('https://gdata.youtube.com/feeds/api/videos?author=%s&orderby=published&max-results=10' % username)
